...
def calc_point2point(predict, actual):
    ...
    try:
        ...
        roc_aus=metrics.aus(fpr,tpr)
        print("our roc_aus")
        plt.title("our roc_aus")
        plt.plot(fpr,tpr,'b',label = 'AUC = %0.2f' %roc_aus)
        plt.show()
        print("end here --------")
    except:
        roc_auc = 0
    #####################################################################################################################
    '''
    pdf = matplotlib.backends.backend_pdf.PdfPages("extra1.pdf")
...
def pot_eval(init_score, score, label, q=1e-5, level=0.02):
    ...
    #####################################################################################################################
    '''
    pdf = matplotlib.backends.backend_pdf.PdfPages("extra2.pdf")
    plt.title("Recall vs Precision")
    plt.plot(p_t[2],label='recall')
    plt.plot(p_t[1],label='precision')
    plt.xticks(rotation=45)
   #plt.set_xlabel('Timestamp')
   #plt.set_xlabel('Timestamp')
    pdf.savefig()
